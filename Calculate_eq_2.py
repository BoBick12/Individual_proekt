...
def solve_quadratic(a, b, c):
    ...
    D = b ** 2 - 4 * a * c
    steps += "Формула дискриминанта: D = b² - 4ac\n"
    steps += f"Этап 1: Вычисление дискриминанта D = ({b})² - 4*({a})*({c}) = {D:g}\n"
    ...
    if D < 0:
        ...
        x = -b / (2 * a)
        steps += "Формула корня: x = -b / 2a\n"
        steps += f"Этап 2: D = 0 \nКорень x = ({-b}) / (2*{a}) = {x:.4f}\nКомплексных корней нет "
        return steps, [x]
    else:
        sd = math.sqrt(D)
        x1, x2 = (-b + sd) / (2 * a), (-b - sd) / (2 * a)
        steps += "Формула корней: x = (-b ± √D) / 2a\n"
        steps += f"Этап 2: Подстановка D > 0 \nКорни: x1 = {x1:.4f}, x2 = {x2:.4f}\nКомплексных корней нет "
        return steps, [x1, x2]
